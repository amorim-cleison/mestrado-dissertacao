...
Em outras palavras, esse foco inadequado faz com que pesquisas em \acrshort{slr} deixem de tratar essa tarefa como um problema de  \acrfull{nlp} e passem a dedicar esforços consideráveis tentando lidar com um conjunto de problemas que a Visão Computacional comumente já aborda ou soluciona em uma de suas subáreas específicas -- os quais incluem a detecção, segmentação e rastreamento de partes do corpo em imagens e vídeos; a interação entre mãos e oclusões decorrentes disso; as variações de tom de pele, cores de roupa e luminosidade do ambiente; entre outros listados nas revisões literárias da \acrshort{slr} elaboradas no decorrer da última década \cite{papastratis-2021-ai-technologies-sl,rastgoo-2021-slr-deep-survey,koller-2020-quantitative-survey-slr,bragg-2019-slr-interdisciplinary,wadhawan-2019-slr-literature-review,suharjito-2018-feature-extraction-survey,joksimoski-2022-scoping-review,cooper-2011-slr}.
...
Se fizéssemos uma analogia com outras tarefas de \acrshort{nlp}, a complexidade de abordar a língua de sinais a partir dessas informações brutas e lidar com todos esses problemas seria equivalente, por exemplo, a tentar reconhecer textos manuscritos rastreando-se o movimento da mão enquanto o autor desenha as letras no papel (ao invés de simplesmente utilizar o texto escrito escaneado como entrada para isso). Ou ainda, tentar reconhecer a fala de um indivíduo apenas rastreando-se os movimentos de seus lábios (ao invés de utilizar os sinais de áudio processados). 
...
Exemplos práticos de que hoje muitos desses problemas já são endereçados consistentemente dentro da Visão Computacional são as ferramentas OpenPose~\cite{wei-2016-conv-machines-openpose,cao-2017-openpose,simon-2017-openpose-hand-face} e MediaPipe~\cite{lugaresi-2019-mediapipe,bazarevsky-2019-mediapipe-blazeface,vakunov-2020-mediapipe-hands,bazarevsky-2020-mediapipe-blazepose}, desenvolvidos pela Carnegie Mellon University e Google Research, respectivamente. 
Ambas são o resultado de anos de pesquisas em torno desses problemas que alcançaram um nível de maturidade elevado capaz de abordar em tempo real tarefas de estimativa de pose e rastreamento do corpo, mãos e face (inclusive envolvendo múltiplos indivíduos) de forma robusta a variações corporais, de luminosidade e de ambientes, utilizando apenas uma câmera simples.
...
} e a reutilização desse conhecimento nas etapas de captura e geração de \textit{features} de níveis mais elevados pelas pequisas em \acrshort{slr} pode conduzir a progressos mais efetivos dessa área.
...
Como resultado do enquadramento inadequado apresentado acima, que se destaca entre os outros desafios que discutimos na \autoref{sec:slr-desafios}, constata-se um progresso muito pouco expressivo da área de \acrshort{slr} no decorrer das últimas décadas, sobretudo sob os aspectos de linguagem e de aplicabilidade no mundo real. 
...
Com base nos argumentos apresentados acima, nesta pesquisa posicionaremos a \acrshort{slr} como uma tarefa de \acrshort{nlp}, delimitando seu escopo ao âmbito da linguística e representando os sinais a partir dos seus componentes linguísticos mínimos -- que são os fonemas, conforme discutido na \autoref{sec:linguistica-lingua-sinais}.
Os fonemas são os parâmetros em termos dos quais as línguas de sinais são formalmente definidas e aprendidas pelos indivíduos.
Com isso, objetivamos eliminar a complexidade pertinente ao escopo da Visão Computacional e concentraremos a capacidade dos modelos aplicados adiante à aprendizagem das regras e restrições linguísticas da língua de sinais.
