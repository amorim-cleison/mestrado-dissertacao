...
À primeira vista, percebe-se que o crescimento do número de estudos com sinais isolados no decorrer dos anos é mais acelerado em comparação àqueles que utilizam sinais contínuos. Isso pode refletir a complexidade mais elevada de reconhecer sinais contínuos e a escassez de \textit{datasets} de treinamento disponíveis para isso. Além disso, os números revelam que a grande maioria dos trabalhos com sinais isolados costumam modelar vocabulários com uma quantidade muito limitada de sinais (principalmente abaixo de 50 sinais), ao passo que os vocabulários modelados por trabalhos com sinais contínuos distribuem-se de forma relativamente uniforme quanto ao tamanho e, com o surgimento de novos \textit{datasets} para essa finalidade em 2015, passaram a se concentrar naqueles maiores (acima de 1000 sinais).
...
A \autoref{tab:tipos-dados-entrada} mostra o tipo de dados de entrada adotados com relação ao total de estudos, organizados em intervalos de cinco anos. Podemos ver que os dados RGB vem sendo utilizados como alternativa desde as primeiras pesquisas nessa área e tornaram-se populares sobretudo a partir de 2005. O uso de dados de profundidade apenas tornou-se popular após o lançamento do sensor Kinect, em 2010. Luvas eletrônicas também tiveram uma participação significativa desde os anos iniciais e foram gradativamente dando espaço para as luvas coloridas, entre 1995 e 2010, quando iniciou-se uma fase de transição para métodos de processamento baseados em visão.

\input{tabelas/fundamentacao/tipos-dados-entrada}


% A Tabela 2 mostra na parte superior o tipo de dados de entrada empregados em diferentes tamanhos de vocabulário modelado. Os dados de entrada referem-se aos dados que são consumidos pelos algoritmos de reconhecimento para extrair recursos e realizar cálculos. Podemos observar que RGB é o tipo mais popular de dados de entrada tanto para intervalos de vocabulário de pequena quanto para maior escala. As luvas coloridas só foram aplicadas em tarefas de vocabulário pequeno e médio e nunca receberam atenção significativa. A parte inferior da Tabela 2 mostra o tipo de dados de entrada empregados em relação a todos os resultados publicados no mesmo intervalo de anos. Podemos ver que os dados RGB atraem mais atenção desde 2005. A profundidade como modalidade de entrada só se tornou popular após o lançamento do sensor Kinect em 2010. Houve um trabalho que empregou dados de profundidade antes [Fujimura e Xia Liu, 2006] que teve acesso a sensores de tempo de voo iniciais. As luvas coloridas ganharam força entre 1995 e 2010, que parece uma fase de transição de dispositivos de medição eletrônicos para processamento baseado em visão pura.
...
Na \autoref{tab:dados-intrusivos-naointrusivos} temos uma comparação dessas técnicas agrupadas segundo as categorias ``intrusiva'' e ``não-intrusiva''. Intrusão, nesse contexto, refere-se à necessidade da técnica interferir no indivíduo articulando o sinal para realizar a estimativa de pose corporal ou extração de características para o reconhecimento. Por exemplo, técnicas baseadas em imagens RGB e dados de profundidade são consideradas não-intrusivas, enquanto que aquelas utilizando luvas (como \textit{Color Glove}, \textit{Electronic Glove} e \textit{Motion Capturing}) são consideradas intrusivas. Percebe-se claramente que houve uma mudança de paradigma após 2005, quando os métodos de captura intrusivos anteriormente dominantes foram cada vez menos utilizados e sua prevalência diminuiu de cerca de 70\% para menos de 30\%, e continuou reduzindo ainda mais ao longo do tempo.

\input{tabelas/fundamentacao/dados-intrusivos-naointrusivos}



% Como pode ser visto na Tabela 3 à esquerda, métodos de captura intrusivos podem ser encontrados em cerca de um quarto de todos os experimentos com um vocabulário de até 500 sinais. Eles são mais raros em tamanhos de vocabulário maiores, possivelmente devido ao fato de terem sido pesquisados ​​principalmente após 2010 (compare a Tabela 1).
