...
\citeonline{quiza-2012-finite-element,russell-2010-modern-approach} definem a \acrfull{ia} como um ramo da ciência da computação que visa estudar e projetar agentes inteligentes, ou seja, sistemas capazes de perceber seu ambiente e realizar ações que maximizam suas chances de sucesso.
A \acrfull{ml}, por sua vez, é definida por \citeonline{murphy-2012-ml-probabilistic,goodfellow-2016-deep-learning} como um conjunto de algoritmos de \acrshort{ia} capazes de detectar automaticamente padrões a partir dos dados e, em seguida, utilizar tais padrões para prever dados futuros ou realizar outros tipos de tomadas de decisões.
...
Segundo \citeonline{mitchell-1997-ml,bishop-2006-pattern-recognition}, essas estruturas são inspiradas em parte pela observação de que sistemas de aprendizado biológico são constituídos de redes muito complexas de neurônios interconectados entre si, conforme ilustra a \autoref{fig:exemplo-neuronios}.
De maneira análoga, as \acrshortpl{ann} são formadas por um conjunto de unidades simples densamente interconectadas organizadas em camadas, que exercem o papel dos neurônios, conforme ilustra a \autoref{fig:rede-neural}. 
...
De acordo com \citeonline{lecun-2015-deep-learning}, a forma mais comum utilizada pelas \acrshortpl{ann} para aprender -- ou adaptar-se aos padrões existentes nos dados -- é o aprendizado supervisionado.
Nela, segundo \citeonline{russell-2010-modern-approach}, a \acrshort{ann} observa um conjunto de exemplos de pares de entrada-saída e aprende uma função que mapeia daquele tipo de entrada para aquela saída respectiva, ajustando seus pesos nesse processo.
...
O passo seguinte compreenderia o treinamento da \acrshort{ann}, processo no qual ela seria exposta às imagens do subconjunto de treinamento para aprender a identificar a existência dos elementos acima. 
...
Segundo \citeonline{lecun-2015-deep-learning,goodfellow-2016-deep-learning}, esse processo de ajuste de pesos é denominado ``otimização'' da \acrshort{ann} e um dos algoritmos mais utilizados atualmente é o \acrfull{sgd}.
...
No entanto, \citeonline{goodfellow-2016-deep-learning,bishop-2006-pattern-recognition} argumentam que dividir o conjunto de dados em apenas dois subconjuntos fixos, de treinamento e de testes, pode ser problemático principalmente se isso resultar em um subconjunto pequeno de testes.
...
Mais adiante nos experimentos deste trabalho, será adotado o algoritmo de seleção de hiperparâmetros conhecido como \textit{grid search} (ou busca de grade).
...
Essas \acrshortpl{dnn} diferenciam-se das redes discutidas até aqui porque apresentam um número muito grande de camadas e, consequentemente, de unidades interconectadas dentro delas -- e justamente por isso são chamadas de ``profundas''.
...
De acordo com \citeonline{jurafsky-2022-speech-lang-processing}, a linguagem é um fenômeno inerentemente temporal que pode ser compreendido como uma sequência de eventos que se desdobram ao longo do tempo, como um fluxo contínuo de dados.
...
Esses redes codificadoras e decodificadoras são comumente implementados utilizando-se \acrshortpl{rnn}, como o \acrshort{lstm} e o \acrshort{gru} introduzidos acima. Além disso, existem otimizações dessa arquitetura que consideram a adição de uma camada de \textit{attention} antes do decodificador com o objetivo de eliminar um gargalo observado ali por \citeonline{bahdanau-2015-mt-align-translate}.
