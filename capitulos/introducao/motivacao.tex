...
O \acrfull{slr} é uma das áreas de pesquisa empenhadas em contribuir para que tecnologias como essas sejam desenvolvidas. Ela é uma área colaborativa que envolve reconhecimento de padrões, visão computacional, processamento de linguagem natural e linguística para construir métodos e algoritmos capazes de identificar sinais produzidos e compreender seu significado~\cite{wadhawan-2021-slr-systems-review}. 
Apesar disso, \citeauthor{cooper-2011-slr} acreditam que o progresso demonstrado por essa área nas últimas décadas ainda foi pouco expressivo:

\begin{quote}
    Enquanto sistemas de reconhecimento da fala avançaram ao ponto de estarem comercialmente disponíveis, o reconhecimento automático de sinais ainda está em sua infância. Atualmente, todos os serviços comerciais de tradução de sinais são baseados em humanos e, portanto, são caros -- devido à necessidade de pessoal especializado~\cite[tradução nossa]{cooper-2011-slr}.
\end{quote}
...
\citeauthor{bragg-2019-slr-interdisciplinary} argumentam que Current research in sign language processing occurs in disciplinary silos (e carecem de uma abordagem interdisciplinar), and as a result does not address the problem comprehensively. For example, there are many computer science publications presenting algorithms for recognizing (and less frequently translating) signed content. The teams creating these algorithms often lack Deaf members with lived experience of the problems the technology could or should solve, and lack knowledge of the linguistic complexities of the language for which their algorithms must account. The algorithms are also often trained on datasets that do not refect real-world use cases. As a result, such single-disciplinary approaches to sign language processing have limited real-world value [39].
...
% Porém, essas línguas vão muito além disso e apresentam desafios particulares, dentre os quais enumeram-se:

\begin{enumerate}
    \item Natureza multicanal: essas línguas transmitem significado através de múltiplos canais ao mesmo tempo -- ou seja, não é suficiente considerar apenas os gestos desenhados pelas mãos no espaço; deve-se também estar atento a suas configurações, intensidade dos movimentos, suas interações com outras partes do corpo, expressões faciais, entre outros traços que compõem os sinais.
    
    \item Linguística particular: já é evidente através dos estudos da linguística das línguas sinalizadas que suas particularidades enquanto línguas visuais tornam muitas das técnicas utilizadas para reconhecimento da fala inadequadas para o \acrshort{slr} -- dessa forma, não é correto simplesmente reproduzir abordagens de processamento de fala; é preciso compreender e desenvolver técnicas que levem em consideração tais particularidades.
    
    \item \textit{Datasets} limitados: os \textit{datasets} de língua de sinais disponíveis publicamente são limitados em quantidade e qualidade, e isso impõe um desafio significativo para o progresso do \acrshort{slr}. Técnicas modernas de aprendizagem de máquina demandam uma quantidade crescente de dados, os quais também precisam ser capazes de replicar o contexto real de aplicação, para resolver problemas de complexidade equivalente à do reconhecimento de sinais.
\end{enumerate}


Em face a esses desafios, o presente trabalho objetiva
